...
def compare_profiles(
    ...
    # Generate summary
    total_comparisons = sum(len(col["comparisons"]) for col in comparison_results.values())
    ...
    comparison_summary = {
        ...
        "significant_changes_percentage": (significant_count / total_comparisons * 100) if total_comparisons > 0 else 0,      
        "deviation_threshold_used": deviation_threshold * 100,
        "baseline_samples": int(baseline_df['baseline_sample_count'].iloc[0]) if len(baseline_df) > 0 else 0,
    ...
    if significant_count > 0:
        ...
        for change in significant_changes[:5]:  # Show first 5
            print(f"  - {change['column_name']}.{change['metric']}: {change['percent_change']:.1f}% change ({change['significance_level']})")
...
def detect_significant_changes(
    ...
    for change in significant_changes:
        row = {
            "comparison_timestamp": bigquery.Client().query("SELECT CURRENT_TIMESTAMP()").to_dataframe().iloc[0, 0],
