...
def protected_table_access(
    ...
    try:
        ...
        """
        filtered_rows = (
            client.query(filtered_query).to_dataframe().iloc[0]["filtered_rows"]
        )
        ...
        row_reduction = (
            ((total_rows - filtered_rows) / total_rows * 100) if total_rows > 0 else 0
        )
        ...
        if filtered_cost < 0.001:
            filtered_display = (
                f"€{filtered_cost:.4f}" if filtered_cost > 0 else "<€0.0001"
            )
        else:
            filtered_display = f"€{filtered_cost:.3f}"

        print(
            f"💰 Cost estimate: €{full_cost:.2f} → {filtered_display} (saves €{cost_savings:.2f})"
        )
        print(
            f"📊 Processing {filtered_rows:,} of {total_rows:,} rows ({row_reduction:.2f}% reduction)"
        )
