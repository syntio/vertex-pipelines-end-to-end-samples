...
class CostAwareBigQueryClient:
    ...
    def __init__(self, project_id: Optional[str] = None):
        ...
        self.cost_warning_threshold = float(
            os.environ.get("BIGQUERY_COST_WARNING_THRESHOLD", "1.0")
        )
        self.require_approval = (
            os.environ.get("BIGQUERY_REQUIRE_APPROVAL", "true").lower() == "true"
        )
        self.auto_approve_ci = (
            os.environ.get("CI", "false").lower() == "true"
        )  # Skip prompts in CI/CD
        self.max_cost_abort = (
            os.environ.get("BIGQUERY_MAX_COST_ABORT", "false").lower() == "true"
        )  # Hard abort on expensive queries
        self.max_cost_limit = float(
            os.environ.get("BIGQUERY_MAX_COST_LIMIT", "50.0")
        )  # Hard limit in production
    ...
    def _request_user_approval(
        self, query: str, estimated_cost: float, bytes_processed: int
    ) -> bool:
        """Request user approval for expensive queries"""
        if self.auto_approve_ci:
            self.logger.warning(
                f"CI environment detected - auto-approving expensive query (€{estimated_cost:.2f})"
            )
            return True
        ...
        while True:
            try:
                response = (
                    input(
                        f"\nProceed with query costing €{estimated_cost:.2f}? [y/N/show]: "
                    )
                    .strip()
                    .lower()
                )
                ...
                if response in ["y", "yes"]:
                    ...
                    print(
                        "Please enter 'y' (yes), 'n' (no), or 'show' to see full query"
                    )
    ...
    def query(
        self, query: str, job_config: Optional[bigquery.QueryJobConfig] = None, **kwargs
    ):
        ...
        if estimated_cost > self.cost_warning_threshold and self.require_approval:
            if not self._request_user_approval(query, estimated_cost, bytes_processed):
                raise ValueError(
                    f"Query cancelled by user - estimated cost €{estimated_cost:.2f} exceeds threshold €{self.cost_warning_threshold}"
                )
