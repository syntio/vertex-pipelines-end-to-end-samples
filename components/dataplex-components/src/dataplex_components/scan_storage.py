...
def store_profile_results(
    ...
    for column_name, metrics in column_metrics.items():
        row = {
            ...
            # Quality metrics
            "null_count": int(
                metrics.get("null_ratio", 0) * table_metrics.get("row_count", 0)
            ),
        ...
        if column_name.lower() in [
            ...
            row["geographic_bounds"] = {
                "min_lat": (
                    metrics.get("min_value")
                    if "latitude" in column_name.lower()
                    else None
                ),
                "max_lat": (
                    metrics.get("max_value")
                    if "latitude" in column_name.lower()
                    else None
                ),
                "min_lon": (
                    metrics.get("min_value")
                    if "longitude" in column_name.lower()
                    else None
                ),
                "max_lon": (
                    metrics.get("max_value")
