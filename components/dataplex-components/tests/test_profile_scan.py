...
def test_run_profile_scan_success(mock_dataplex_profile_result):
    ...
    with mock.patch(
        "google.cloud.dataplex_v1.DataScanServiceClient"
        ...
        mock_client.get_data_scan.side_effect = Exception(
            "Not found"
        )  # First call fails, triggers creation
...
def test_run_profile_scan_existing_scan(mock_dataplex_profile_result):
    ...
    with mock.patch(
        ...
        assert (
            profile_results["scan_metadata"]["pipeline_stage"] == "post-preprocessing"
        )
