...
def test_compare_profiles_with_baseline(sample_profile_results):
    ...
    baseline_data = {
        ...
        "baseline_mean": [
            40.0,
            None,
        ],  # Current: 45.5, Baseline: 40.0 = 13.75% increase
        "baseline_stddev": [
            12.0,
            None,
        ],  # Current: 15.2, Baseline: 12.0 = 26.67% increase
    ...
    with mock.patch("google.cloud.bigquery.Client") as mock_bq_client:
        ...
        comparison_results, significant_changes, comparison_summary = (
            compare_profiles.python_func(
        ...
        assert mean_comparison["is_significant"] is True
        ...
        assert stddev_comparison["is_significant"] is True

        # Verify significant changes detected
        assert len(significant_changes) > 0

        trip_total_changes = [
            c for c in significant_changes if c["column_name"] == "trip_total"
        ]
...
def test_compare_profiles_no_baseline(sample_profile_results):
    ...
    with mock.patch("google.cloud.bigquery.Client") as mock_bq_client:
        ...
        comparison_results, significant_changes, comparison_summary = (
            compare_profiles.python_func(
                current_profile=sample_profile_results, project_id="test-project"
            )
        )
        ...
        assert comparison_summary["first_scan"] is True
...
def test_compare_profiles_deviation_thresholds(sample_profile_results):
    ...
    with mock.patch("google.cloud.bigquery.Client") as mock_bq_client:
        ...
        mean_significant_changes = [
            c for c in significant_changes if c["metric"] == "mean_value"
        ]
        assert len(mean_significant_changes) == 1
        assert (
            mean_significant_changes[0]["significance_level"] == "WARNING"
        )  # 13.75% < 20%
...
def test_detect_significant_changes_continue():
    ...
    with mock.patch("google.cloud.bigquery.Client") as mock_bq_client:
        ...
        mock_client.query.return_value.to_dataframe.return_value = pd.DataFrame(
            {"current_timestamp": ["2024-01-15T10:30:00"]}
        )
